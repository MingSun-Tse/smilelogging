...
all_exps_ = [x for x in glob.glob(f'{args.exps_folder}/{args.kw}') if os.path.isdir(x) and 'SERVER' in x]
if len(all_exps_) == 0:
...
if args.exact_kw:
    ...
    for exp in all_exps_:
        ...
        if exp_name not in exp_groups:
            exp_groups.append(exp_name)
        all_exps_with_the_same_name = glob.glob(f'{args.exps_folder}/{exp_name}_*')
